...
function comprimirUrls(urls, reemplazos){
	for( var u in urls ){
		var url = urls[u][0];
		
		for( var r in reemplazos ){
			url = url.replace(reemplazos[r][0] + "/", reemplazos[r][1] + "/");
		}
		url = url.replace(/\.gif$/gi, "");
		
		document.write("[\"" + url + "\", " + urls[u][1] + "],<br />");
	}
}

comprimirUrls(urls, reemplazos);