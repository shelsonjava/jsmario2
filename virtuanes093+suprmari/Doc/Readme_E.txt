...
VirtuaNES version 0.93 (C)2001-2006 Norix
...
VS-Unisystem                    Support(Color map is not perfect. VS-Dualsystem 
                                        does not correspond.)

Screen filter                   Support(MMX is requisite)
...
Edit(E)
        ...
        ---------------
        Quick load              Load by choosing slot of State file directly.
        Quick save              Save by choosing slot of State file directly.
        ---------------
        Movie                   Can display playback and a record of a movie, 
                                information.
                                Conversion to AVI of a movie.
        ---------------
...
Tool(T)
        ...
        -----------------
        Auto IPS patch(I)       When it is checked, an IPS patch is applied at 
                                the same time to open a ROM file.
        -----------------
...
Launcher is started simultaneously with a startup
        It is option which starts launcher simultaneously with startup of 
        VirtuaNES.
...
Picture of snapshot is taken by PNG
        Take a picture of snapshot not by BMP but by PNG.

Use JoystickID
        It is recognized in Windows rightly, but when Joystick isn't recognized 
        in VirtuaNES, you are made ON, and please test it.

	Note) Restart of VirtauNES is necessary.

Pause at launcher active
        It is the option which does a pause of emulation at active time of a 
        launcher.
...
Using Sleep
        It is mitigable that synchronous gap may be caused.
        Keep in mind that speed of game may become amusing if this option is 
        turned OFF.
        Note) Please do not use, when screen frequency is except 60Hz, if it 
              is NTSC.
...
Display also except 4:3
        Please turn ON to use resolution, for example, 1280x1024dot.
...
Only setting change
        Only change of setting performs this button without saving setting to INI.
        Please use, when you test setting.
...
Upper row shows fixed palette of NES. It will choose, if it left-clicks with mouse.

Bottom of it shows state of BG palette and SP palette. A left-click chooses fixed 
palette in use.

Color Info
        Each parameter of number (hexadecimal number) of color and RGB chosen is 
        displayed.
        Edit each parameter of RGB with editing box or slider.

UNDO
        UNDO can be carried out only once at the time of editing color.
        REDO (UNDO of UNDO) is not made.

Right click
        Choose movement of right-click.
...
Zapper is also common setting.

Coin1		C key
Coin2		unsetting
Service		unsetting

DipSwitch sets up by [Edit]-> [ExtDevice]-> [VS-Unisystem DipSwitch] of menu.
...
Perform DipSwitch setting in game of VS-Unisystem.
Don't memorize state of DipSwitch but after startup always uses default setting.
Please refer to manual of game, if attached to the contents of setting.
...
Note) Since there was list selection from v0.85, it changed so that file list 
      might not be updated automatically, even if it changed registration folder.
...
================================
= Automatic IPS patch function =
================================

An automatic IPS patch applies an IPS patch inside at the same time to open a 
ROM file.

An IPS file is established in an IPS folder by the name that turned extension 
of a ROM file name into ".IPS".

An IPS file in an archive can be applied that a ROM file will be put in a ZIP 
archive with an IPS file.(ZIP only)

Note) When a program is reconstructed, state data of the thing which isn't 
      reconstructed are loaded, and there isn't security when the reverse was 
      done.
Note) The IPS file name in a ZIP archive didn't need to be the same from 0.90a.

===========================
= AVI conversion function =
===========================

Please refer to AVIconv.txt.
...
Using converting 2xSaI/Super2xSaI/SuperEagle(Derek Liauw Kie Fa product) and 
Scale2x(Advanced Project) and hq2x/lq2x(Maxim Stepin product. Source code from 
Advanced Project).
...
A mail is this one.(please delete NOVIRUS!)
